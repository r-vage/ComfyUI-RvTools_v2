...
class RvText_Prompt_Environment:
    ...
    def load_json(self):
        ...
        try:
            ...
            log.error("PromptEnvironment", f"Error reading JSON file: {e}")  
    ...
    def get_input_types_from_keys(keys):
        input_types = {}
        for key in keys:
            input_types[key] = (tuple(RvText_Prompt_Environment.get_options_keys(key)), {"default": "None"})
        return input_types
    ...
    def get_options_keys(key):
        ...
        parent_dir = os.path.abspath(os.path.join(current_dir, '..'))
        json_dir = os.path.join(parent_dir, 'json')  
        json_file_path = os.path.join(json_dir, RvText_Prompt_Environment.JSON_FILE_PATH)
...
NODE_NAME = 'Prompt Environment [RvTools]'
NODE_DESC = 'Prompt Environment'

NODE_CLASS_MAPPINGS = {
   NODE_NAME: RvText_Prompt_Environment
}
