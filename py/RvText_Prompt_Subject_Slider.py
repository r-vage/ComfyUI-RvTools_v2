...
class RvText_Prompt_Subject_Slider:
    ...
    def load_json(self):
        ...
        try:
            ...
            log.error("PromptSubjectSlider", f"Error reading JSON file: {e}")  
    ...
    def get_input_types_from_keys(keys):
        input_types = {}
        for key in keys:
            input_types[key] = (tuple(RvText_Prompt_Subject_Slider.get_options_keys(key)), {"default": "None"})
    ...
    def get_options_keys(key):
        ...
        parent_dir = os.path.abspath(os.path.join(current_dir, '..'))
        json_dir = os.path.join(parent_dir, 'json')  
        json_file_path = os.path.join(json_dir, RvText_Prompt_Subject_Slider.JSON_FILE_PATH)
...
NODE_NAME = 'Prompt Subject Slider [RvTools]'
NODE_DESC = 'Prompt Subject Slider'

NODE_CLASS_MAPPINGS = {
   NODE_NAME: RvText_Prompt_Subject_Slider
}
