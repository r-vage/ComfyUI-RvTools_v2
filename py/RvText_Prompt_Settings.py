...
class RvText_Prompt_Settings:
    ...
    def get_input_types_from_keys(keys):
        input_types = {}
        for key in keys:
            input_types[key] = (tuple(RvText_Prompt_Settings.get_options_keys(key)), {"default": "None"})
        return input_types
    ...
    def get_options_keys(key):
        ...
        parent_dir = os.path.abspath(os.path.join(current_dir, '..'))
        json_dir = os.path.join(parent_dir, 'json')  
        json_file_path = os.path.join(json_dir, RvText_Prompt_Settings.JSON_FILE_PATH)
...
NODE_NAME = 'Prompt Settings [RvTools]'
NODE_DESC = 'Prompt Settings'

NODE_CLASS_MAPPINGS = {
   NODE_NAME: RvText_Prompt_Settings
}
